...
export class WETH9 extends _Contract{
    ...
    withdraw: {
        ...
        txData: (wad:number|BigNumber, options?: TransactionOptions) => Promise<string>;
    }
    private assign(){
        ...
        let deposit_txData = async (options?: number|BigNumber|TransactionOptions): Promise<string> => {
            let result = await this.txData('deposit',[],options);
            return result;
        }
        ...
        let withdraw_txData = async (wad:number|BigNumber, options?: TransactionOptions): Promise<string> => {
            let result = await this.txData('withdraw',[this.wallet.utils.toString(wad)],options);
            return result;
        }
